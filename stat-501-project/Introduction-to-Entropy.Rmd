...
$$ H(x) = - \sum_{i = 1}^{c} f(x)logF(x) $$

Importantly, the logarithm may be taken with regard to any base. Commonly, $e$, 10, and 2, with entropy being measured in units of 'nats', 'dits', or 'dits', respectively. 

### Definition of Conditional Entropy

Entropy can also be measured given another event. For a continuous random variable, given Y:

$$ H^Y(X) = - \int f_{X | Y}(x|y)(log(f_{X | Y}(x|y))dx $$

For a discrete random variable: 

$$ H(x) = - \sum_{i = 1}^{c} f_{X | Y}(x|y)logf_{X | Y}(x|y) $$
...
In a simple GWAS, the genotypes of individuals affected by a disease (cases) are compared to those of individuals unaffected by a disease (controls). In these scenarios, entropy may be used as a measure of the diversity in either group's collective genomes. Actually, Shannon's Entropy is used as an index for diversity in ecology, known as the Shannon index. In a GWAS, If the diversity at SNP sites differs between case and control genomes, the association of a disease with the SNPs of interest can be tested. 

# Extension of example

 For a set of two SNP locations, M1 and M2, let there be two alleles, A and a, and B and b, respectively. 
